...
def find_actor(query, cast) :
    ...
    """
    for entry in cast:
        if query.lower() in entry['person']['name'].lower() or query.lower() in entry['character']['name'].lower() :
            return entry
    return None
    
def format_actor_info(dict) :
    """
    Format the actor's information
    """
    return f"{dict['person']['name']} ({dict['person']['gender']}, born {dict['person']['birthday']} in \
{dict['person']['country']['name']}) plays {dict['character']['name']}"
...
def main():
    ...
    while query != "quit": #added a way to end loop
        query = input("Search for an actor or character: ")
        actor = find_actor(query, cast)
        if actor:
            print(f"Found: {format_actor_info(actor)}")
        else:
